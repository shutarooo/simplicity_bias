...
def norm_initialize(model):
    ...
    for i in range(len(model.linear_relu_stack)):
        if isinstance(model.linear_relu_stack[i], nn.Linear):
            model.linear_relu_stack[i].weight = nn.Parameter(model.linear_relu_stack[i].weight * (9.0/norm))
            model.linear_relu_stack[i].bias = nn.Parameter(model.linear_relu_stack[i].bias * (9.0/norm))
