...
while True:
    ...
    if last_loss < loss:
        if is_overfitting == 0:
            torch.save(model, 'model_10_input/model_10.pth')
        is_overfitting += 1
    else:
        is_overfitting = 0
    last_loss = loss
    ...
    for i in range(len(tensor_input)):
        output = torch.heaviside(model(tensor_input[i])-0.5, torch.tensor([0.0], device=device))
...
path_file = 'model_10_input/info_10.json'
with open(path_file, 'w') as f:
    json.dump({'diff': diff_list, 'norm': norm_list, 'comp_process': epoch_comp, 'loss_process': loss_list, 'target_function': dataset.labels.tolist(), 'epochs': epochs, 'train_size': 112, 'val_size': 16, 'threshold': 0.9} , f)
